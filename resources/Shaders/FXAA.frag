...
vec4 FXAA(sampler2D screenTexture, vec2 inUV)
{
	...
	float lumaDown = rgb2luma(textureOffset(screenTexture,inUV,ivec2(0,-1)).rgb);
	float lumaUp = rgb2luma(textureOffset(screenTexture,inUV,ivec2(0,1)).rgb);
	float lumaLeft = rgb2luma(textureOffset(screenTexture,inUV,ivec2(-1,0)).rgb);
	float lumaRight = rgb2luma(textureOffset(screenTexture,inUV,ivec2(1,0)).rgb);
	...
	if(lumaRange < max(EDGE_THRESHOLD_MIN,lumaMax*EDGE_THRESHOLD_MAX)){
		return vec4(colorCenter, 0);
	}
	
	// Query the 4 remaining corners lumas.
	float lumaDownLeft = rgb2luma(textureOffset(screenTexture,inUV,ivec2(-1,-1)).rgb);
	float lumaUpRight = rgb2luma(textureOffset(screenTexture,inUV,ivec2(1,1)).rgb);
	float lumaUpLeft = rgb2luma(textureOffset(screenTexture,inUV,ivec2(-1,1)).rgb);
	float lumaDownRight = rgb2luma(textureOffset(screenTexture,inUV,ivec2(1,-1)).rgb);
