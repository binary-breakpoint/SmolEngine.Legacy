...
#endif
{
	...
	void VulkanContext::BeginFrameEx(VkCommandBuffer cmdBuffer)
	{
		// Get next image in the swap chain (back/front buffer)
		VK_CHECK_RESULT(m_Swapchain.AcquireNextImage(m_Semaphore.GetPresentCompleteSemaphore()));
	}

	void VulkanContext::SwapBuffersEx(VkCommandBuffer cmdBuffer)
	{
		const auto& present_ref = m_Semaphore.GetPresentCompleteSemaphore();
		const auto& render_ref = m_Semaphore.GetRenderCompleteSemaphore();
		constexpr uint64_t DEFAULT_FENCE_TIME_OUT = 100000000000;
		const VkFence fence = m_Semaphore.GetVkFences()[m_Swapchain.GetCurrentBufferIndex()];
		...
		VK_CHECK_RESULT(vkResetFences(m_Device.GetLogicalDevice(), 1, &fence));
		...
		VK_CHECK_RESULT(vkWaitForFences(m_Device.GetLogicalDevice(), 1, &fence, VK_TRUE, DEFAULT_FENCE_TIME_OUT));
	}
